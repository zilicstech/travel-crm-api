...
check "§7.3 no entities returned from controllers" 0 \
  "$(grep -rlE 'ResponseEntity<(List<)?(Lead|LeadMember|LeadNote|LeadProposal|LeadTimeline|LeadService|LeadFollowUp|LeadVoucher|AgencySetting|Client|Member|MemberDocument|Booking|Visa|Agent|Tenant|ClientInvoice|SupplierInvoice|PlatformAdmin)[>,]' $SRC/controller/ 2>/dev/null | wc -l | tr -d ' ')"
